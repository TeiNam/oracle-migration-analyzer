...
class SGAAdviceFormatter:
    ...
    def _format_ko(data: StatspackData) -> str:
        ...
        current_sga = next(
            (a for a in data.sga_advice if abs(a.sga_size_factor - 1.0) < 0.01), None
        )
        
        # 최적 SGA 찾기 (DB Time이 가장 낮은 것)
        optimal_sga = min(data.sga_advice, key=lambda x: x.estd_db_time)
