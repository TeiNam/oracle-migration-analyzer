...
| 항목 | 값 | 설명 |
|------|-----|------|
| 데이터베이스 이름 | SUPPLY_CHAIN_MGMT | 마이그레이션 대상 DB를 식별하는 이름입니다 |
| Oracle 버전 | 19.0.0.0.0 | 현재 사용 중인 Oracle 버전입니다. 버전에 따라 사용 가능한 기능이 다릅니다 |
| DBID | 4567890123 | Oracle이 내부적으로 사용하는 고유 식별자입니다 |
...
| 항목 | 값 | 설명 |
|------|-----|------|
| 전체 DB 크기 | 380.0 GB | 실제 데이터가 차지하는 디스크 공간입니다. 중간 규모로 일반적인 마이그레이션 절차를 적용합니다 |
...
- ℹ️ **중간 규모 DB**: 380GB 규모로 일반적인 마이그레이션 절차를 적용합니다
...
|-------------|------|------------|------|
| 패키지 | 8 | 🔴 높음 | 여러 프로시저/함수를 묶은 모듈. 가장 복잡한 변환 대상 |
| 프로시저 | 45 | 🟠 중간 | 특정 작업을 수행하는 프로그램 단위 |
| 함수 | 25 | 🟠 중간 | 값을 계산하여 반환하는 프로그램 단위 |
| 트리거 | 245 | 🟠 중간 | 데이터 변경 시 자동 실행되는 프로그램 |
| 타입 | 45 | 🔴 높음 | 사용자 정의 데이터 타입. 타겟 DB에서 재설계 필요 |
| 타입 바디 | 1 | 🔴 높음 | 타입의 메서드 구현부. 타입과 함께 변환 필요 |
| **총 PL/SQL 오브젝트 수** | **369** | - | 변환 대상 프로그램 오브젝트 총합 |
| **총 PL/SQL 라인 수** | **50,000** | - | 변환 작업량 산정의 기준 |
...
- **예상 변환 시간**: 약 **167시간** (100줄당 20분 기준)
- **AI 도구 활용 시**: 약 **100시간** (40% 단축 가능)
- **예상 인력**: 20.8인일 (1일 8시간 기준)
...
| 오브젝트 유형 | 개수 | 변환 방법 | 설명 |
|-------------|------|----------|------|
| 스키마 | 18 | SCT 자동 변환 | 데이터베이스 내 논리적 구분 단위 |
| 테이블 | 680 | SCT 자동 변환 | 실제 데이터가 저장되는 기본 단위 |
| 뷰 | 285 | 일부 수동 검토 | 가상 테이블. 복잡한 뷰는 검토 필요 |
| 인덱스 | 1,850 | SCT 자동 변환 | 검색 속도 향상을 위한 구조 |
| 시퀀스 | 185 | SCT 자동 변환 | 자동 증가 번호 생성기 |
| LOB | 125 | SCT 자동 변환 | 대용량 데이터 (이미지, 문서 등) |
| DB Link | 2 | 아키텍처 검토 | 다른 DB 연결. 네트워크 구성 재설계 필요 |
...
| 메트릭 | 값 | 의미 |
|--------|-----|------|
| 평균 CPU/s | 28.83 | 일반적인 부하 상태 |
| 최소 CPU/s | 25.50 | 가장 한가한 시점 |
| 최대 CPU/s | 32.50 | 가장 바쁜 시점 (피크) |
...
| 메트릭 | 읽기 | 쓰기 | 합계 | 의미 |
|--------|------|------|------|------|
| 평균 IOPS | 48 | 16 | 64 | 일반적인 디스크 사용량 |
| 최대 IOPS | 52 | 18 | 71 | 피크 시 디스크 사용량 |
| 평균 처리량 (MB/s) | 6.7 | 83.5 | 90.2 | 데이터 전송 속도 |
...
| 메트릭 | 값 | 의미 |
|--------|-----|------|
| 평균 커밋/초 | 1.46 | 초당 완료되는 트랜잭션 수 |
| 최대 커밋/초 | 1.73 | 피크 시 트랜잭션 처리량 |

### 마이그레이션 시사점

> 위 성능 데이터를 바탕으로 분석한 AWS 인스턴스 선택 가이드입니다.

- 📖 **읽기 비중이 높음 (74%)**: 조회 작업이 많은 시스템입니다. Aurora Read Replica를 활용하면 읽기 성능을 분산시킬 수 있습니다.
...
**요약:**
- **총 스냅샷 수**: 3개
- **평균 메모리 사용량**: 49.83 GB (SGA: 40.00 GB, PGA: 9.83 GB)
- **최소/최대**: 48.50 GB / 51.20 GB

**상세 데이터 (최근 10개):**

| Snap ID | Instance | SGA (GB) | PGA (GB) | Total (GB) |
|---------|----------|----------|----------|------------|
| 40001 | 1 | 40.00 | 8.50 | 48.50 |
| 40002 | 1 | 40.00 | 9.80 | 49.80 |
| 40003 | 1 | 40.00 | 11.20 | 51.20 |
...
| 현재 예상 DB Time | 145,000 | 현재 설정에서의 예상 처리 시간 |
| 현재 예상 Physical Reads | 20,000,000 | 현재 설정에서의 예상 디스크 읽기 횟수 |
| **권장 SGA 크기** | **50 MB** | **최적 성능을 위한 권장 크기** |
| 권장 시 예상 DB Time | 125,000 | 권장 설정에서의 예상 처리 시간 |
| 권장 시 예상 Physical Reads | 16,000,000 | 권장 설정에서의 예상 디스크 읽기 횟수 |
| **예상 성능 개선** | **13.8%** | **DB Time 감소율** |
...
|---------------|-------------|--------------|---------------------|------|
| 30 | 0.75 | 165,000 | 25,000,000 |  |
| 40 | 1.00 | 145,000 | 20,000,000 | 현재 |
| 50 | 1.25 | 125,000 | 16,000,000 | ⭐ 권장 |

## 💿 디스크 사용량 통계

**요약:**
- **평균 디스크 사용량**: 380.00 GB
- **최소/최대**: 380.00 GB / 380.00 GB
...
|------|------------|------------|-----------|-------------|
| 1 | DB | CPU DB CPU | 82.2% | 5,050,000,000초 |
| 2 | User | I/O db file sequential read | 7.5% | 450,000,000초 |
| 3 | Commit | log file sync | 4.4% | 265,000,000초 |
| 4 | Other | PGA memory operation | 2.8% | 165,000,000초 |
| 5 | System | I/O log file parallel write | 1.8% | 110,000,000초 |
...
| ADDM | 4 | 🟠 | 검토 필요 |
| AWR Report | 4 | 🟠 | 검토 필요 |
| Internode Parallel Execution | 35 | 🟠 | 검토 필요 |
| Job Scheduler | 62 | 🟠 | 검토 필요 |
| LOB | 68 | 🟠 | PostgreSQL: BYTEA/TEXT, MySQL: BLOB/TEXT |
| Parallel SQL Query Execution | 55 | 🟠 | 검토 필요 |
| Real-Time SQL Monitoring | 48 | 🟠 | 검토 필요 |
| Recovery Manager (RMAN) | 70 | 🟠 | 검토 필요 |
| Result Cache | 45 | 🟠 | 검토 필요 |
...
| 기능 | 사용 횟수 |
|------|----------|
| Locally Managed Tablespaces (system) | 70 |
| Oracle Java Virtual Machine (system) | 70 |
| SecureFiles (system) | 70 |
...
|------|-----|------|
| 인스턴스 | 1 (단일) | 1 = 단일 |
| PL/SQL 라인 수 | 50,000 🟠 | < 20,000 |
| 프로시저 수 | 45 | < 50 |
| 함수 수 | 25 | < 30 |
| 패키지 수 | 8 | < 20 |
| DB 크기 | 380.0 GB | < 500 GB |
| DB Link 수 | 2 | < 5 |
| 쓰기 IOPS (최대/평균) | 18 / 16 | - |

### 📋 판단 근거

- 중간 규모 PL/SQL (50,000줄, 20,000~100,000줄)
