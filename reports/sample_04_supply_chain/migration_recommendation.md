# SUPPLY_CHAIN_MGMT Oracle 마이그레이션 전략 리포트

---

## 리포트 개요

**추천 타겟**: Aurora PostgreSQL

**추천 신뢰도**: 75%

**소스 데이터베이스**: SUPPLY_CHAIN_MGMT (Oracle 19.0.0.0.0)
...
| 항목 | 값 | 설명 |
|------|-----|------|
| 전체 DB 크기 | 380.0 GB | 중소 규모로 일반적인 마이그레이션 절차를 적용합니다 |
...
| **총 PL/SQL 오브젝트 수** | **78** | - | 변환 대상 프로그램 오브젝트 총합 |
...
**Refactoring (리팩토링)** → Aurora PostgreSQL
...
| 항목 | PostgreSQL | MySQL |
|------|------------|-------|
| 평균 복잡도 | 3.11 (낮음) | 4.39 (중간) |
| 최대 복잡도 | 6.55 (높음) | 7.15 (높음) |
| 고난이도 | 1개 / 5개 (≥5.0) | 1개 / 4개 (≥7.0) |

> **분석**: MySQL 타겟이 PostgreSQL보다 복잡도가 **1.28** 높습니다. PostgreSQL이 Oracle 호환성이 더 좋아 변환이 용이합니다.
...
|----------|------|------|--------|----------|
| PL/SQL 평균 복잡도 | <3.0: 0점, 3.0~5.0: 1점, 5.0~7.0: 2점, ≥7.0: 3점 | 0~3 | 3.11 (중간 (3.0~5.0)) | 1점 |
| PL/SQL 코드량 | <10K: 0점, 10K~50K: 1점, 50K~100K: 2점, ≥100K: 3점 | 0~3 | 50,000줄 (많음 (50K~100K)) | 2점 |
| 고난이도 비율 (≥5.0) | <10%: 0점, 10~30%: 1점, ≥30%: 2점 | 0~2 | 1/5개 (20.0%) | 1점 |
| 고난이도 절대 개수 | <20: 0점, 20~50: 1점, 50~100: 2점, ≥100: 3점 | 0~3 | 1개 (적음 (<20개)) | 0점 |
| 패키지 개수 | <50: 0점, 50~100: 1점, ≥100: 2점 | 0~2 | 8개 (적음 (<50)) | 0점 |
| **합계** | - | **0~13** | - | **4점** |
...
- ⚠️ PL/SQL 코드량이 많음 (50,000줄)
- ⚠️ 고난이도 오브젝트 1개 존재 (복잡도 ≥5.0)

# 인스턴스 추천

> **이 섹션의 목적**: AWR/Statspack 성능 데이터를 기반으로
> Aurora PostgreSQL에 적합한 **AWS 인스턴스 타입**을 추천합니다.
...
**추천 근거**: 예상 피크 메모리(59.8GB)를 고려하여 **db.r6i.2xlarge** (vCPU 8, 메모리 64GB)를 추천합니다. Aurora PostgreSQL의 자동 스케일링 기능으로 피크 시 추가 확장이 가능합니다.
...
|------|---------------------|-------------------|
| **인스턴스 타입** | db.r6i.2xlarge | db.r6i.2xlarge |
| **vCPU** | 8 | 8 |
| **메모리** | 64 GB | 64 GB |
...
### 고려사항
- ✅ PL/SQL 복잡도가 낮아 애플리케이션 이관 상대적으로 용이
...
## 위험 1: PL/SQL 5개를 PL/pgSQL로 변환 필요 (일부 기능 미지원)

**카테고리**: technical

**심각도**: medium

**완화 방안**: AWS SCT 자동 변환 활용, 미지원 기능(패키지 변수, PRAGMA) 사전 식별

## 위험 2: 고복잡도 오브젝트 비율(20.0%)이 높아 테스트 기간 증가 예상
...
## 위험 3: 운영팀 PostgreSQL 학습 및 운영 체계 구축 필요
...
## 위험 4: 중첩 테이블(NESTED TABLE) 1회 사용 - 변환 필요

**카테고리**: technical

**심각도**: medium

**완화 방안**: ARRAY 또는 별도 테이블로 분리
...
| 평균 PL/SQL 복잡도 | 3.11 | 0~10 척도 |
| 고난이도 SQL | 0개 | ≥5.0 기준 |
| 고난이도 PL/SQL | 1개 | ≥5.0 기준 |
| BULK 연산 | 0개 | 성능 영향 |
| 평균 CPU | 28.8% | AWR 기준 |
| 평균 I/O | 64.0 IOPS | AWR 기준 |
| 평균 메모리 | 49.8 GB | AWR 기준 |

## 요약

| 항목 | 내용 |
|------|------|
| **추천 전략** | Aurora PostgreSQL (Refactoring) |
